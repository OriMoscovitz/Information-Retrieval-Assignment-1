...
- "qid" (query ID, a string), 
- "docno" (document number, a string appearing in the DOCNO tags in the documents), 
- "rank" (integer starting from 0), 
- "sim" (similarity score, a float) and 
- "run_id" (identifying the system/run name, must be same for one file). 
- The "iter" (string) field is ignored and unimportant for this assignment.
